...
(SSO) and Single Log Off (SLO) in your deployment. Each @sharepoint@ instance
...
| **Note:** A single @sharepoint@ instance is *either* the SP or the IdP in your SSO
...
The `5.0.0` version of the application for @sharepoint@ brings some long-awaited
improvements:

* @sharepoint@ acting as a Service Provider (SP) can now connect to multiple
...
@sharepoint@ reads certificates from the SAML metadata XML file. If you want a
third-party application like Salesforce to read a Liferay SAML certificate, you
can export the @sharepoint@ certificate from the keystore. The default keystore
file is 
...
configured in @sharepoint@, using the SAML Web Browser SSO profile as defined in
...
If @sharepoint@ is the IdP, the IdP initiated SSO URL
...
For non-@sharepoint@ IdPs (Siteminder, ADFS, etc.), consult the vendor's
...
on setting up the SPC in @sharepoint@'s SAML adapter).
...
| metadata XML provided by the SP. In @sharepoint@, you import the SP's metadata in the
| SAML Adapter's Service Provider Connections tab.

#### The SP Processes the SSO Response

The SP validates and processes the SAML Response. @sharepoint@'s SAML solution
...
- If one @sharepoint@ instance is the IdP and another is the
  SP, make sure the SAML metadata XML file imported into the SP contains the
  IdP's certificate. 
- If @sharepoint@ is the IdP and another application is the SP,
...
it triggers the SP initiated SSO process. When @sharepoint@ is the SAML SP, SSO is
...
the `RelayState`. For non-@sharepoint@ SPs, consult the vendor documentation on
initiating SSO.

#### The AuthnRequest to the IdP

The SP looks up the IdP's Single Sign On service URL and sends an
`AuthnRequest`. When @sharepoint@ is the SP it looks up the configured SAML Identity
Provider Connection and sends a SAML `AuthnRequest` to the IdP's Single Sign On
service URL as defined in the SAML metadata XML document. @sharepoint@ supports
...
the SP, the user must authenticate by providing credentials. When @sharepoint@ is
the IdP, authentication occurs in the Login Portlet. @sharepoint@ decodes and
...
When @sharepoint@ is configured as the IdP, any attributes configured on the Service
...
When @sharepoint@ is configured as the SP, response and assertion signatures are
verified. @sharepoint@ requires the sender to be authenticated. This is done via
...
non-@sharepoint@ SP or IdP vendors, consult their documentation.
...
When @sharepoint@ is the IdP, the IdP initiated SSO URL must specify the URL
path as 
...
non-@sharepoint@ IdPs (Siteminder, ADFS, etc.) consult the vendor's documentation on
constructing IdP initiated SLO URLs.

The IdP sends a SAML `LogoutRequest` to the SP.

- When @sharepoint@ is configured as the IdP, the `LogoutRequest` is sent using
  ...
  binding is selected.
- When @sharepoint@ is configured as the SP, supported bindings for `LogoutRequest`
...
repeated for all SPs the user is logged into. When @sharepoint@ is the IdP,
@sharepoint@ logs the user out after the last SP has delivered its `LogoutResponse`
or has timed out.
...
SP. When @sharepoint@ is configured as the SP, the SLO is initiated by requesting
this logout URL:
...
-  If @sharepoint@ serves as the SP, the `LogoutRequest` is sent to the IdP
...
-  When @sharepoint@ is the IdP, if the user has logged on to other SPs, the user
...
IdP sends the SAML `LogoutRequest` to each one. When @sharepoint@ is the IdP, the
`LogoutResponse` is sent using either HTTP-Post, HTTP-Redirect, or SOAP binding.

Each SP delivers its `LogoutResponse` to the IdP. When @sharepoint@ is the SP, the
...
destroys its SSO session. When @sharepoint@ is the IdP, once the last SP has
delivered its `LogoutResponse` or has timed out, the IdP destroys the @sharepoint@
