...
# Setting up @sharepoint@ as a SAML Service Provider

[TOC levels=1-4]

Many of these steps are similar to configuring @sharepoint@ as a SAML Identity
Provider. As a reminder, a single @sharepoint@ installation can be configured as a
SAML Identify Provider *or* as a SAML Service Provider but not as both. If your
@sharepoint@ installation is already a SAML Identity Provider, use a *different*
@sharepoint@ installation as a SAML Service Provider.

| **Note:** If you have a third party IdP with @sharepoint@ as the SP, all
| messages coming from the IdP must be signed. If they're not, an error message
| appears and communication between the IdP and @sharepoint@ fails.
...
2.  To begin configuring @sharepoint@ to use SAML, you must select a SAML role for
    @sharepoint@ and choose an entity ID. Select the *Service Provider* SAML role.
...
5.  Next, you must configure an Identity Provider connection. Click on the
    *Identity Provider Connections* tab. Enter a name for the Identity Provider,
    ...
    followed the previous instructions and configured a separate @sharepoint@
    ...
    **Important**: The Liferay Connector to SAML 2.0 app supports using *either*
    ...
    XML file. Otherwise, @sharepoint@ keeps the original metadata URL in the
...
6.  Finally, after you save your certificate and private key information and
    ...
    of the General tab and click *Save*. @sharepoint@ is now a SAML Service Provider!
...
2. Verify the connection to the IdP. 
    ...
    b.  *Entity ID*: the same name of the IdP. If the IdP is another @sharepoint@
        instance, then it is the same name as the above example. 

    c. *Metadata URL*: The IdP's metadata as a URL or as an XML file.

    d. If the IdP is another @sharepoint instance, ensure its corresponding
...
## Setting Up @sharepoint@ as a SAML Service Provider in a Clustered Environment

You can use the Liferay Connector to SAML 2.0 app as an SSO solution for a
clustered @sharepoint@ environment. If your multi-node cluster is behind a load
...
1.  Configure each node of your 
    [@sharepoint@ cluster](/docs/7-2/deploy/-/knowledge_base/d/liferay-clustering) 
...
4.  At this point, all  nodes have the same SAML SP configuration and each of
    ...
    SSO solution, sign into @sharepoint@ via your load balancer, navigate to a few
...
Now you know how to configure @sharepoint@ either as a SAML identity provider
