---
header-id: upgrading-the-sharepoint-data
---

# Upgrading the @sharepoint@ Data

[TOC levels=1-4]

Now you're ready to upgrade the @sharepoint@ data. The upgrade processes update the
...
those versions. Since version 7.0, however, @sharepoint@'s modular framework lets
...
| upgrades for fix packs. Since @sharepoint@ 7.1, a fix pack's micro upgrade
...
| [back up the @sharepoint@ database, installation, and Document Library store](/docs/7-2/deploy/-/knowledge_base/d/backing-up-a-liferay-installation). 
| 
