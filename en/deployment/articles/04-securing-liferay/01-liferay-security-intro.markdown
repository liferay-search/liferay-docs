...
@sharepoint@ is as secure as possible. Following these recommendations protects the
sharepoint against known kinds of attacks and security vulnerabilities. For
example, @sharepoint@'s persistence layer is generated and maintained by the
...
output. To support integration features, @sharepoint@ doesn't encode input. Data is
stored in the original form as submitted by the user. @sharepoint@ includes built-in
...
To stay on top, @sharepoint@ also contains fixes for state-of-the-art attacks and
techniques to improve sharepoint security. For example, @sharepoint@ uses PBKDF2 to
store passwords. @sharepoint@ also contains mitigation for Quadratic Blowup XXE
...
@sharepoint@ user authentication can take place using any of a variety of prepared
solutions:
...
- Remote IP and HTTPS transport check to limit access to @sharepoint@'s Java
  servlets
...
| Note: @sharepoint@ relies on the application server for sanitizing CRLF in
...
| this case, @sharepoint@ sanitizes HTTP headers.
...
- Configure @sharepoint@'s HTTPS web server address
...
@sharepoint@ is built using the "secure by default" concept in mind. It's not
...
[customer portal](https://www.liferay.com/group/customer/sharepoints/portal/security-vulnerability).
