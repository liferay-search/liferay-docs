...
*Liferay Home* is the location from which @sharepoint@ launches applications, 
applies configurations, loads JAR files, and generates logs. 

-   *In a @sharepoint@ bundle,* Liferay Home is the installation's top-level folder
...
-   *In a manual installation,* the Liferay Home folder varies by application
    ...
    covering that app server (e.g., *Installing @sharepoint@ on [app server]*) for
...
-   **[Liferay Home]**
    ...
    -   `license`: @sharepoint@'s copyright and version files are here.
    ...
    -   `tools`: For @sharepoint@ upgrade and target platform indexer.
    -   `work`: Module Jasper work files.

| **Note:** If @sharepoint@ cannot create resources in the 
...
| operating system user that is running @sharepoint@. In this case, that `liferay`
