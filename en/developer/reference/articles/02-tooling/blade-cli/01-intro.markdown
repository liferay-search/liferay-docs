...
- Create/manage @sharepoint@ instances
...
`deploy` | Builds and deploys projects to @sharepoint@. See the [Deploying a Project section for Blade CLI](/docs/7-2/reference/-/knowledge_base/r/deploying-a-project#blade-cli) for more information.
...
`sh` | Connects to @sharepoint@, executes succeeding Gogo command, and returns output. For example, `blade sh lb` lists @sharepoint@'s bundles using the Gogo shell. See the [Managing Your Liferay Server with Blade CLI](/docs/7-2/reference/-/knowledge_base/r/managing-your-liferay-server-with-blade-cli) article for more information.
...
`upgradeProps` | Analyzes your old `portal-ext.properties` and your newly installed 7.x server to show you properties moved to OSGi configuration files or removed from the sharepoint. 
...
Continue on to learn about leveraging Blade CLI to create and test @sharepoint@
