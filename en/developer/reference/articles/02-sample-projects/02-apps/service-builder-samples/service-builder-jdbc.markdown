...
2.  Create a `portal-ext.properties` file in your @sharepoint@ instance's
    ...
    Note that @sharepoint@'s primary data source is specified by the `jdbc.default`
...
After these modules have been deployed, add the `-web` portlet to a @sharepoint@
page.
...
the rows in the `country` table to @sharepoint@'s log.
